...
public class AMapUtil {
	...
	//将手机定位获取的坐标转换成高德坐标
	public static LatLng trunsformLatLng(Context context, String latitude, String longitude){
		CoordinateConverter converter = new CoordinateConverter(context);
		converter.from(CoordinateConverter.CoordType.GPS);
//			converter.from(CoordinateConverter.CoordType.GOOGLE);
		LatLng latLng = new LatLng(Double.parseDouble(latitude),Double.parseDouble(longitude));
		converter.coord(latLng);
		com.amap.api.maps.model.LatLng newLatLng=null;
		try {
			newLatLng = converter.convert();
			return newLatLng;
//			double aMapLatitude = newLatLng.latitude;
//			double aMapLongitude = newLatLng.longitude;
		} catch (Exception e) {
			e.printStackTrace();
		}
		return null;
