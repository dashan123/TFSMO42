...
public class ViewPagerFragmentB64Photo extends Fragment {
	...
	public ViewPagerFragmentB64Photo() {
	}
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		if (rootView == null) {
			rootView = inflater.inflate(layout.view_pager_fragment_b64_photo, container, false);
		}
		...
		if (bundle != null) {
			if (currStr == null && bundle.containsKey("currStr")) {
				...
				if (imageView==null) {
					imageView = (SubsamplingScaleImageView) rootView.findViewById(id.imageView_b64_photo);
				...
				if (currStr != null) {
					LogUtils.d("currStr = "+currStr);

					bytes=Base64.decode(currStr, Base64.DEFAULT);
					LogUtils.d("bytes      "+bytes);

					// 如果SD卡已挂载
					if (SDCardHelper.isSDCardMounted()) {
						dirFile = new File(SDCardHelper.getSDCardPath() + File.separator + "/workPicturePhoto/");
						if (!dirFile.exists()) {
							dirFile.mkdirs();
						}
					} else {
