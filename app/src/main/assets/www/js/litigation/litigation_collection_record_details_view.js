/************************litigation_collection_record_details_view_page init begin*****************************************************************/	
var litigation_collection_record_details_view_page = $('#litigation_collection_record_details_view_page');
var litigation_collection_record_details_view_myScroll;
/******************************home_page---begin**************************************/	   
litigation_collection_record_details_view_page.live('pageinit',function(e, ui){
	var wrapper = "litigation_collection_record_details_view_wrapper";
	litigation_collection_record_details_view_myScroll = createMyScroll(wrapper);
	
	litigation_collection_record_details_view_page.find(".BackBtn").live("tap",function(event){
	...
	//跳转到 案件详情  事件
	litigation_collection_record_details_view_page.find(".bottoms1-red li:first").live("tap",function(){
		
		goto_page("collection_case_details_view_page");
	});
	
    litigation_collection_record_details_view_page.find('[tag="picBox"] img').live("tap",function(){
...
litigation_collection_record_details_view_page.live('pageshow',function(e, ui){
	
	currentLoadActionName = "litigation_collection_record_details_view_load_content"
	
	var fromPage = session.get("fromPage");
	 if(fromPage != "collection_case_details_view_page" && fromPage != "image_zoom_page"){
			load_litigation_collection_record_details_view_content();
	 }

});

function litigation_collection_record_details_view_load_content(){
	
}

//加载详细，并显示
function load_litigation_collection_record_details_view_content(){
	
	showLoading();
	var currentPage = $("#litigation_collection_record_details_view_page");
