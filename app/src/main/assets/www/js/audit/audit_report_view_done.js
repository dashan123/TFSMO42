var audit_report_view_done_page = $('#audit_report_view_done_page');

var audit_report_view_done_myScroll;

/******************************audit_report_view_done_page---begin**************************************/
audit_report_view_done_page.live('pageinit',function(e,ui){
	
	var wrapper = "audit_report_view_done_wrapper";
	var up = "audit_report_view_done_pullUp";
	var down = "audit_report_view_done_pullDown";
	audit_report_view_done_myScroll = createMyScroll(wrapper,up,down);
	
	//回退事件处理
	audit_report_view_done_page.find(".BackBtn").live("tap",function(event){
		event.stopPropagation();
		back_page();
	});
	
	//点击 全部/已盘/未盘/公里数抽查 按钮，检索满足条件的车辆信息显示到页面
	audit_report_view_done_page.find(".auditSegmentedButtonGroup input[type='button']").live("tap",function(event){
		event.stopPropagation();
//		var audiSelectedBtn = $('#audit_report_view_done_page').find(".auditSegmentedButtonGroup .segmentedButtons4Selected");
...
//		$(this).addClass("segmentedButtons4Selected");
		...
		var $vinNoText = $('#audit_report_view_done_page').find("#vinNoText_view_done");
		var $vinNoVal = $vinNoText.val();
		queryAuditReportViewDone(queryConditonVal,$vinNoVal);
	});
	
	//点击数字键盘
	audit_report_view_done_page.find(".auditReportViewNumericKeyboardNumber").live("tap",function(event){
		event.stopPropagation();
		//查询盘点报告
		...
		if($vinNoText.val().length >= 4){
			var queryConditonVal = $('#audit_report_view_done_page').find(".auditSegmentedButtonGroup .segmentedButtons4Selected").val();
			var queryVinNoVal = $vinNoText.val();
			queryAuditReportViewDone(queryConditonVal,queryVinNoVal);
		}
	});
	
	//点击删除按钮，删除检索条件车架号文本框中的1个字符，然后进行模糊查询
	audit_report_view_done_page.find("#deleteVinNo_view_done").live("tap",function(event){
		event.stopPropagation();
		//查询盘点报告
		var $vinNoText = $('#audit_report_view_done_page').find("#vinNoText_view_done");
		var $vinNoVal = $vinNoText.val();
	...
	audit_report_view_done_page.find("#clearVinNo_view_done").live("tap",function(event){
		event.stopPropagation();
		//查询盘点报告
		var $vinNoText = $('#audit_report_view_done_page').find("#vinNoText_view_done");
		$vinNoText.val("");
...
audit_report_view_done_page.live('pageshow',function(e, ui){

	currentLoadActionName  = "audit_report_view_done_load_content";
	//查询提示消息列表
	load_audit_report_view_done_content();
	
});//end pageshow

function audit_report_view_done_load_content(){
	//下拉不刷新，则该方法置空
}

//初始化盘点报告页面
function load_audit_report_view_done_content(){
	var page = $('#audit_report_view_done_page');
	var $auditReportList = page.find(".auditReportViewContentDiv .auditReportViewList");
	$auditReportList.empty();
	
	var auditReportStatisticsNumFont = $("#audit_report_view_done_page .auditReportStatisticsNum font");
	auditReportStatisticsNumFont.text("0");
	
//	var $auditSegmentedButtonGroup = $('#audit_report_view_done_page').find(".auditSegmentedButtonGroup input");
//	var $auditReportTotalBtn = $('#audit_report_view_done_page').find(".auditSegmentedButtonGroup #auditReportTotalBtn_view_done");
...
//	$auditReportTotalBtn.addClass("segmentedButtons4Selected");
	...
	$.getJSON(basePath+"/app/auditReportViewDone/pageInit.xhtml"+callback, postData,function(msg){
...
//			var qcFinishNum = 0;  //合格证已盘数量
		    ...
		    if(data.auditReportList !=null && data.auditReportList.length > 0){
		    	//盘点报告初始化
		    	//加载盘点报告列表
		    	var auditReportList = data.auditReportList;
				var $template = $("#audit_report_view_done_page .list-row-template");
				$.each(auditReportList,function(i,n){
					var $item = $template.clone(true);
					
					var $auditReportViewDonePhotoBtn = $item.find("a.auditReportEditPhotoBtn");
					$auditReportViewDonePhotoBtn.bind("tap",function(event){
						event.stopPropagation();
						var auditCheckReportId = $(this).parents(".auditReportViewListItem").find("[identity='auditCheckReportId']").text();
						var auditCheckListId = $(this).parents(".auditReportViewListItem").find("[identity='auditCheckListId']").text();
						var vinNo = $(this).parents(".ListTit").find("[identity='vinNo']").text();
						auditReportViewDonePhotoBtn(auditCheckReportId,auditCheckListId,vinNo);
					});
...
//					}
					...
					$item.find("[identity='auditCheckListId']").text(data.auditCheckListId);
					
					//加载销售日期
					if(n.auditResults=="OFFSE" || n.auditResults=="SOT" || n.auditResults=="SOLD"){
						$item.find("#auditReportViewDone_soldDateDiv").css("display","");
					}else{
						$item.find("#auditReportViewDone_soldDateDiv").css("display","none");
					}
				...
				page.find("#auditReportTotalNum_view_done font").text(auditReportTotalNum);
//				page.find("#auditFinishNum_view_done font").text(auditFinishNum);
//				page.find("#auditUnfinishNum_view_done font").text(auditUnfinishNum);
//				page.find("#odomSelectiveNum_view_done font").text(odomSelectiveNum);
//				page.find("#qcFinishNum_view_done font").text(qcFinishNum);
...
//查询盘点报告
function queryAuditReportViewDone(queryConditonVal,vinNoVal){
	var page = $('#audit_report_view_done_page');
	var $auditReportList = page.find(".auditReportViewContentDiv .auditReportViewList");
	$auditReportList.empty();
	
//	var auditReportStatisticsNumFont = $('#audit_report_view_done_page').find(".auditReportStatisticsNum font");
//	auditReportStatisticsNumFont.text("0");
...
//    }
	...
	$.getJSON(basePath+"/app/auditReportViewDone/queryAuditReportList.xhtml"+callback, postData,function(msg){
		if($.trim(msg.returnCode) == '0'){
...
//点击图片链接处理
function auditReportViewDonePhotoBtn(auditCheckReportId,auditCheckListId,vinNo){
	if(auditCheckReportId != null && auditCheckReportId != ""){
		...
		$.getJSON(basePath+"/app/auditReportViewDone/queryAuditCheckReportId.xhtml"+callback, postData,function(msg){
			if($.trim(msg.returnCode) == '0'){
				showHide();
				var data = msg.data;
			    if(data !=null){
			    	session.set("businessId",data.auditCheckReportId);
					goto_page("common_business_pictures_view_page");
			    }
