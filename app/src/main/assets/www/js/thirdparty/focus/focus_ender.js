...
!function (context) {
  ...
  // Implements Ender's $_ global access object
  // =========================================
  ...
  aug(ender, {
    ...
    fn: context.$_ && context.$_.fn || {} // for easy compat to jQuery plugins
  });
  ...
  aug(boosh, {
    ...
    $_: ender // handy reference to self
  });
  ...
  context['ender'] = context['$_'] = context['ender'] || ender;
...
!function () {
  ...
  !function (name, definition) {
    ...
    function interpret(whole, tag, idsAndClasses, wholeAttribute, attribute, qualifier, value, wholePseudo, pseudo, wholePseudoVal, pseudoVal) {
      ...
      if (idsAndClasses && (classes = idsAndClasses.match(clas))) {
        for (i = classes.length; i--;) {
          c = classes[i].slice(1)
          if (!(classCache.g(c) || classCache.s(c, new RegExp('(^|\\s+)' + c + '(\\s+|$_)'))).test(this.className)) {
            return false
          }
        }
      }
    ...
    function clean(s) {
      return cleanCache.g(s) || cleanCache.s(s, s.replace(specialChars, '\\$_1'))
    }
    ...
    function checkAttr(qualify, actual, val) {
      ...
      case '$_=':
        return actual.match(attrCache.g('$_=' + val) || attrCache.s('$_=' + val, new RegExp(clean(val) + '$_')))
      case '*=':
        return actual.match(attrCache.g(val) || attrCache.s(val, new RegExp(clean(val))))
      case '~=':
        return actual.match(attrCache.g('~=' + val) || attrCache.s('~=' + val, new RegExp('(?:^|\\s+)' + clean(val) + '(?:\\s+|$_)')))
      case '|=':
        return actual.match(attrCache.g('|=' + val) || attrCache.s('|=' + val, new RegExp('^' + clean(val) + '(-|$_)')))
      }
      return 0
    }
    ...
    function _qwery(selector) {
      ...
      intr = q(token)
      els = dividedTokens && /^[+~]$_/.test(dividedTokens[dividedTokens.length - 1]) ? function (r) {
    ...
    select = supportsCSS3 ?
      ...
      function (selector, root) {
        selector = selector.replace(normalizr, '$_1')
        var result = [], element, collection, collections = [], i
        if (m = selector.match(tagAndOrClass)) {
          items = root.getElementsByTagName(m[1] || '*');
          r = classCache.g(m[2]) || classCache.s(m[2], new RegExp('(^|\\s+)' + m[2] + '(\\s+|$_)'));
