/*********************************************************login init begin*****************************************************************/	
var collection_record_details_page = $('#collection_record_details_page');
var collection_record_details_myScroll;
var collection_record_details_page_handler = {};
/******************************home_page---begin**************************************/	   
collection_record_details_page.live('pageinit',function(e, ui){
	var wrapper = "collection_record_details_wrapper";
	collection_record_details_myScroll = createMyScroll(wrapper);
	
	collection_record_details_page.find(".BackBtn").live("tap",function(event){
		event.stopPropagation();
		back_page();
	});
	
	//跳转到 案件详情  事件
	collection_record_details_page.find(".bottoms1-red li:first").live("tap",function(){
		
		goto_page("collection_case_details_view_page");
	});
	
	//点击保存
	collection_record_details_page.find(".SaveBtn1").live("tap",function(){
		collection_record_details_page_handler.saveCollectionRecord();
	});
	
	//点击提交
	collection_record_details_page.find(".SaveBtn2").live("tap",function(){
		collection_record_details_page_handler.submitCollectionRecord();
	});
	collection_record_details_page_init();
	//点击图片查看大图
    collection_record_details_page.find('[tag="picBox"] img').live("tap",function(){
...
//    	}
    	...
    	var currentPage = $("#collection_record_details_page");
...
function collection_record_details_page_init(){
	//点击催收对象跳转页面进行选择
	var $collect_object_li = $("#collection_record_details_collect_object");
	collection_record_details_page_handler.collect_object_li_tap_handler=function(){
		var dictData = {};
	...
	$collect_object_li.on("tap",collection_record_details_page_handler.collect_object_li_tap_handler);
	
	//点击联系方式跳转页面进行选择
	var $contact_method_li = $("#collection_record_details_contact_method");
	collection_record_details_page_handler.contact_method_li_tap_handler=function(){
	...
	$contact_method_li.on("tap",collection_record_details_page_handler.contact_method_li_tap_handler);
	
	
	//点击行动代码跳转页面进行选择
	var $action_code_li = $("#collection_record_details_action_code");
	collection_record_details_page_handler.action_code_li_tap_handler=function(){
	...
	$action_code_li.on("tap",collection_record_details_page_handler.action_code_li_tap_handler);
	
	//点击行动状态跳转页面进行选择
	var $action_status_li = $("#collection_record_details_action_status");
	collection_record_details_page_handler.action_status_li_tap_handler=function(){
	...
	$action_status_li.on("tap",collection_record_details_page_handler.action_status_li_tap_handler);
	
}

collection_record_details_page.live('pageshow',function(e, ui){
	
	currentLoadActionName = "collection_record_details_load_content"
	...
	if (fromPage == "collection_dictionary_data_list_page") {
		...
		load_collection_record_details_content();
	}
	
});

function collection_record_details_load_content(){
	
}

//加载详细，并显示
function load_collection_record_details_content(){
...
//    postData.caseRecordId =session.get("caseRecordId");
	$.getJSON(basePath+"/app/collectionRecordDetails/pageInit.xhtml"+callback, postData,function(msg){
...
//					if(collectionRecord.status != ConstDef.getConstant("COLLECTION_RECORD_STATUS_UN_SAVE")){
						...
						$.each(contractMethodList,function(i,n){
							if(n.code == collectionRecord.contractMethod){
								var contractMethodList = $("#collection_record_details_contact_method");
						...
						if(collectionRecord.status == "2"){
							...
							currentPage.find("[id=collection_record_details_collect_object] span:last").addClass("allBtn_down");
							currentPage.find("[id=collection_record_details_contact_method] span:last").addClass("allBtn_down");
							currentPage.find("[id=collection_record_details_action_code] span:last").addClass("allBtn_down");
							currentPage.find("[id=collection_record_details_action_status] span:last").addClass("allBtn_down");
...
collection_record_details_page_handler.saveCollectionRecord=function(){

	var page = $('#collection_record_details_page');
	   ...
	   showLoading();
	   $.getJSON(basePath+"/app/collectionRecordDetails/saveCollectionRecord.xhtml"+callback,postData,function(msg){
...
collection_record_details_page_handler.submitCollectionRecord=function (){
//	if(!validateCollectionRecordForm()){
//		return;
//	}
	var page = $('#collection_record_details_page');
...
//	   postData.collectionRecordId = session.get("collectionRecordId");
	...
	showConfirm("请确认已保存当前页面数据，提交后将不能再进行更改", function(){
		...
		//保存成功后返回已保存的催记ID
		var $currentPage =$("#collection_record_details_page");
//		var collectionRecordId = $currentPage.find(".SaveBtn1").attr("collectionRecordId");
		...
		postData.contract_number = $currentPage.find("[identity='contract_number']").text();
		   ...
		   $.getJSON(basePath+"/app/collectionRecordDetails/submitCollectionRecord.xhtml"+callback,postData,function(msg){
...
/*//页面数据非空校验
function validateCollectionRecordForm(){
	   var page = $('#collection_record_details_page');
