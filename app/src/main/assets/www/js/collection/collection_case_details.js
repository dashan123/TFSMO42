/*********************************************************login init begin*****************************************************************/	
var collection_case_details_page = $('#collection_case_details_page');
var collection_case_details_myScroll;
/******************************home_page---begin**************************************/	   
collection_case_details_page.live('pageinit',function(e, ui){
	
	var wrapper = "collection_case_details_wrapper"
	var up = "collection_case_details_pullUp";
    var down = "collection_case_details_pullDown";;
	collection_case_details_myScroll = createMyScroll(wrapper,up,down);
	
	//内部合同号，从其它画面传入并将传递给 催收记录，催收照片，还款计划，扣款流水 功能画面使用
//	console.log(session.get("contractId"));

	collection_case_details_page.find(".BackBtn").live("tap",function(event){
		event.stopPropagation();
		back_page();
	});
	
	//点击展开/关闭列表
	collection_case_details_page.find("div.case-details h1").live("tap",function(e){
	...
	//跳转到扣款流水页面
	collection_case_details_page.find("div.case-details[url='case_debit_list_page'] h1").live("tap",function(e){
		goto_page("case_debit_list_page");
	});
	
	//跳转到还款计划页面
	collection_case_details_page.find("div.case-details[url='case_repayment_plan_list_page'] h1").live("tap",function(e){
		goto_page("case_repayment_plan_list_page");
	});
	
	//跳转到核销还款明细
	collection_case_details_page.find("div.case-details li em[id='collectionCaseDetail_showWriteOffDetail']").live("tap",function(e){
		session.set("page_keyword","案件详情");
		session.set("page_title","案件详情");
		session.set("page_from","collection_case_details_page");
		goto_page("case_writeoff_repayment_list_page");
	});
	
	//催收信息--点击来电记录按钮，进入催收信息的来电记录列表页面
	collection_case_details_page.find("div.case-details li em[id='collectionCaseDetail_showCollInfo']").live("tap",function(e){
		goto_page("collection_caller_record_list_page");
	});
});


collection_case_details_page.live('pageshow',function(e, ui){
	currentLoadActionName = "collection_case_details_load_content";
	//如果网络是连通的
	if(isNetworkConnected()){
		...
		key.method = "load_collection_case_details_content";
		var keyExtra = {};
		...
		extra.callback = "load_collection_case_details_content_from_native_storage";
		extra.newDataKey = {};
		extra.newData = {};
...
function collection_case_details_load_content(){
	//该方法被引用，不要删除，如果想实现向上划动刷新，则在此添加代码
}
//加载详细，并显示
function load_collection_case_details_content(){

		showLoading();
	
		var currentPage = $("#collection_case_details_page");
        ...
        $.getJSON(basePath+"/app/collectionCaseDetails/pageInit.xhtml"+callback, postData,function(msg){
			if ($.trim(msg.returnCode) == '0') {
				...
				if (msg.data != null) {
					...
					bind_collection_case_details__to_page(msg);
...
//加载详细，并显示
function load_collection_case_details_content_from_native_storage(value){
...
function bind_collection_case_details__to_page(msg){
	var currentPage = $("#collection_case_details_page");
...
/*	if(collCaseContractInfo != null){
	...
	$newAddressElement.find("em").bind("tap",
			{type:"newAddress",value:$newAddressElement},addCollectingAddress);
...
function addCollectingAddress(e){
		...
		postData.todoCaseId = session.get("cssCollectionRecordDetailsSave_toDoCaseId");
		...
		//业务类型：1催收 2诉讼
		postData.businessFlag = "1";
		
		showConfirmDialog("请确认是否将地址添加到在催地址列表！",postData, function(){
			showLoading();
			$.getJSON(basePath+"/app/collectionCaseDetails/addCollectingAddress.xhtml"+callback, postData,function(msg){
				if ($.trim(msg.returnCode) == '0') {
					showHide();
