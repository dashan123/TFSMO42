/*********************************************************login init begin*****************************************************************/	
var collecting_address_details_page = $('#collecting_address_details_page');
var collecting_address_details_myScroll;
var collecting_address_details_page_handler = {};
var deletedFiles = new Array();//要删除的图片文件id

collecting_address_details_page_handler.collecting_address_details_page_init = function(){
	//点击催收对象跳转页面进行选择
	var $collect_object_li = $("#collecting_address_details_collect_object");
	...
	//点击联系方式跳转页面进行选择
	var $contact_method_li = $("#collecting_address_details_contact_method");
	...
	//点击行动代码跳转页面进行选择
	var $action_code_li = $("#collecting_address_details_action_code");
	...
	//点击行动状态跳转页面进行选择
	var $action_status_li = $("#collecting_address_details_action_status");
	...
	//拍照取证
	var $currentPage = $('#collecting_address_details_page');
	$currentPage.find(".bottoms2-red li:last").on("tap",function(){
		...
		if (ConstDef.getConstant("ONLINE_STATUS_SIGNIN") == userInfo.user.onlineStatus) {
			...
			setTimeout(function(){
				showHide();
				startCamera(0.7,"onCameraDone4CollectingAddressDetails");
			},1000)
		}
		else{
...
/******************************home_page---begin**************************************/
collecting_address_details_page.live('pageinit',function(e, ui){
	var wrapper = "collecting_address_details_wrapper";
//	var up = "collecting_address_details_pullUp";
//	var down = "collecting_address_details_pullDown";
	collecting_address_details_myScroll = createMyScroll(wrapper);

	collecting_address_details_page.find(".BackBtn").live("tap",function(event){
	...
	//跳转到 案件详情  事件
	collecting_address_details_page.find(".bottoms2-red li:first").live("tap",function(){
		//传递参数contractId :session.set("contractId",msg.data.caseRecord.contractId);
		goto_page("collection_case_details_view_page");
	});

	//点击保存
	collecting_address_details_page.find(".SaveBtn1").live("tap",function(){
		showConfirm("请确认数据是否无误，无误后确认保存", function(){
//			$(this).attr("disabled","disabled");
			console.log("保存催记！！！！！");
			saveCollectionRecord();
//			$(this).removeAttr("disabled");  //移除属性
		});
	});

	//点击提交
	collecting_address_details_page.find(".SaveBtn2").live("tap",function(){

		submitCollectionRecord();

	});

	collecting_address_details_page_handler.collecting_address_details_page_init();
	//点击删除图片
	collecting_address_details_page.find('[tag="picBox"] dd').live("tap",function(){
	...
	//图片点击放大
	collecting_address_details_page.find('[tag="picBox"] img').live("tap",function(){  
//		showMessage($(this).attr("src").replace("data:image/jpeg;base64,",""),'10000');
//		var fileId = $(this).attr("attachmentId");
//		if(fileId){
//			session.set("fileId",fileId);//获取附件ID
//			
//			//caseId 已在加载页面时赋值
//			goto_page("collection_css_picture_details");
//		}
//		else{
//			var newStr = $(this).attr("src").replace("data:image/jpeg;base64,","");
...
collecting_address_details_page.live('pageshow',function(e, ui){

	currentLoadActionName = "collecting_address_details_load_content";
	...
	if (fromPage == "collection_dictionary_data_list_page") {
		...
		if(isNetworkConnected()){
			load_collecting_address_details_content();
		}
		else{
			...
			key.fun = "collecting_address_details_page";
			key.method = "load_collecting_address_details_content";
			keyExtra = {};
			...
			extra.callback = "load_collecting_address_details_content_from_native_storage";
			extra.newDataKey = {};
			extra.newData = {};
...
function collecting_address_details_load_content(){

}

//清空页面数据
function clear_collecting_address_details_content(){
	var currentPage = $("#collecting_address_details_page");
	...
	collecting_address_details_default_values_set();
}

//设置页面元素默认值
function collecting_address_details_default_values_set(){
	var $collect_object_li = $("#collecting_address_details_collect_object");
	var $collect_object = $collect_object_li.find("[identity='collect_object']")
...
//	var $collect_object_li = $("#collecting_address_details_collect_object");
	...
	$("#collecting_address_details_page").find("[identity='next_time']").val(tenDaysLatter);
}
//加载详细，并显示
function load_collecting_address_details_content(){
	//清空页面数据
	clear_collecting_address_details_content();
	...
	$.getJSON(basePath+"/app/collectingAddressDetails/pageInit.xhtml"+callback, postData,function(msg){
		...
		if($.trim(msg.returnCode) == '0'){
			...
			bind_collecting_address_details_to_page(msg);	
...
function load_collecting_address_details_content_from_native_storage(value){
...
function bind_collecting_address_details_to_page(msg){
...
//	currentPage.find("[identity='collectionRecordStatus']").val(collectionRecord.status);
	if(collectionRecord != null && collectionRecord.status != ConstDef.getConstant("COLLECTION_RECORD_STATUS_UN_SAVE")){
		...
		$.each(contractMethodList,function(i,n){
			if(n.code == collectionRecord.contractMethod){
				var contractMethodList = $("#collecting_address_details_contact_method");
...
//		currentPage.find("[identity]").unbind();
...
//			currentPage.find(".bottoms2-red li:last").hide();
			...
			currentPage.find("[id=collecting_address_details_collect_object] span:last").addClass("allBtn_down");
			currentPage.find("[id=collecting_address_details_contact_method] span:last").addClass("allBtn_down");
			currentPage.find("[id=collecting_address_details_action_code] span:last").addClass("allBtn_down");
			currentPage.find("[id=collecting_address_details_action_status] span:last").addClass("allBtn_down");
...
//			currentPage.find(".bottoms2-red li:last").show();
...
function saveCollectionRecord(){
	var page = $('#collecting_address_details_page');
...
//	postData.contract_number = page.find("[identity='contract_number']").text();
	...
	//获取要上传的图片及要删除的图片
	var collecting_address_details_page = $('#collecting_address_details_page');
	var $images = collecting_address_details_page.find('[tag="picBox"] img')

	//标记是否已有拍摄催记照片
//	session.set("hasPictures",$images.length>0);
	...
	if(isNetworkConnected()){
		...
		//上传催记数据
		$.ajax({
			url: basePath+"/app/collectingAddressDetails/upload.xhtml", //这个地址做了跨域处理
			data: formData,
			processData: false,
			contentType: false,
			type: 'POST',
				...
				if($.trim(msg.returnCode) == '0') {
					...
					if(msg.data){
						if(msg.data.result == "success"){
							...
							var collecting_address_details_page = $('#collecting_address_details_page');
							collecting_address_details_page.find('[tag="picBox"] img').attr("status","saved");
		...
		var fun = "collecting_address_details";
		var method = "saveCollectionRecord";
		var action = basePath+"/app/collectingAddressDetails/saveCollectionRecord.xhtml";
		var data = postData;
...
function saveCollectionRecordToNativeStorage(){
	...
	//取得所有要上传的图片
	var collecting_address_details_page = $('#collecting_address_details_page');
	var $images = collecting_address_details_page.find('[tag="picBox"] img:visible')
...
function submitCollectionRecord(){
...
//		var contract_number = page.find("[identity='contract_number']").text();
		...
		//上传催记数据
		$.ajax({
			url: basePath+"/app/collectingAddressDetails/submitCollectionRecord.xhtml", 
			data: formData,
			processData: false,
			contentType: false,
			type: 'POST',
				...
				if($.trim(msg.returnCode) == '0') {
					if(msg.data){
						if(msg.data.result == "success"){
							...
							//保存成功后返回已保存的催记ID
							collecting_address_details_page.find(".SaveBtn1").attr("collectionRecordId",msg.data.collectionRecordId);
						}
					}
...
//--------------------
//相机拍照的回调
function onCameraDone4CollectingAddressDetails(src) {
	showLoading();
	...
	.then(function(url) {
		...
		.then(function (rst) {
			...
			collecting_address_details_page.find('[tag="picBox"]').append($item);
		})
		.catch(function (err){
